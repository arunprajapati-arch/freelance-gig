        ...
        <div className="w-full max-w-9xl mx-auto px-4">
            <div className="flex flex-wrap gap-4 md:gap-8 2xl:gap-12 justify-center">
                {testimonials.map((testimonial, index) => (
                    <div
                        key={index}
                        className="flex-shrink-0 rounded-lg p-4 md:p-6   w-full sm:w-80 2xl:w-[26rem]"
                    >
                        <div className="flex items-center gap-1 mb-2">
                            {[...Array(testimonial.rating)].map((_, i) => (
                                <Star key={i} className="w-3 h-3 md:w-4 md:h-4 fill-red-700 text-red-700" />
                            ))}
                        </div>
                        ...
                        <div className="border-t border-stone-200 pt-2">
                            ...
                            <p className="text-red-900 text-xs font-medium">{testimonial.role}</p>
                        </div>
                    </div>
                ))}
            </div>
